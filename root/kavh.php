...
<div class="abu-section" style="padding: 50px; display: inline-block;">
    <div class="abu-container">
        <div class="abu-title">
            <h1 class="abu-h1">Kurumsal Amaç ve Hedeflerimiz</h1>
        </div>
        <div class="abu-content">
            <div class="abu-article" style="font-size: 18px;">
                <?php
                    $sorgu_m = $veritaConn -> prepare("SELECT kurumsalAmacVeHedefler FROM temelyazi");
                    $sorgu_m -> execute();
                    $met = $sorgu_m -> fetch(PDO::FETCH_ASSOC);
                    if($met){ echo $met["kurumsalAmacVeHedefler"]; }
                ?>
            </div>
        </div>
