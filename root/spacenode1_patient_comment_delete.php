<?php
    ...
    if($queryErrInfo[0] != "00000"){
        $_SESSION["message_main"] = 'Hata!';
        $_SESSION["message_comment"] = 'Yorum Bilgisi Alınamadı. İşlem sırasında beklenmeyen bir hata oluştu. Lütfen daha sonra tekrar deneyiniz.';
    ...
    unlink($resim["resimYolu"]);
    $query_yorumSil = $veritaConn -> prepare("DELETE FROM hastayorum WHERE id = ?");
    $query_yorumSil -> execute([$incoming_id]);
    $queryErrInfo = $query_yorumSil -> errorInfo();
    if($queryErrInfo[0] != "00000"){
        $_SESSION["message_main"] = 'Hata!';
        $_SESSION["message_comment"] = 'Yorum Silinemedi. İşlem sırasında beklenmeyen bir hata oluştu. Lütfen daha sonra tekrar deneyiniz.';
        $_SESSION["message_routing"] = '<a href="index.php?mdl=6&mdlsp1=15">Geri Dön</a>';
        $_SESSION["image_path"] = '<h2 style="color: rgba(255, 0, 0, 0.4);"><i class="fa-solid fa-circle-xmark"></i></h2>';
        header("Location:index.php?mdl=2");
        exit();
    ...
    $_SESSION["message_comment"] = 'Yorum Başarıyla Silindi!';
    $_SESSION["message_routing"] = '<a href="index.php?mdl=6&mdlsp1=15">Geri Dön</a>';
